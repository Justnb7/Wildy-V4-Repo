	...
	public void patchObjectInteraction(final int objectId, final int itemId, final int x, final int y) {
		...
		if (objectId == FarmingConstants.GRASS_OBJECT || objectId == FarmingConstants.HERB_PATCH_DEPLETED) {
			if (player.getFarmingState(id) < State.RAKED.getId()) {
				...
				//if (player.getSkills().getLevelForExperience(player.getExperience[Skills.FARMING]) < herb.getLevelRequired()) {
					//player.getActionSender().sendMessage("You need a farming level of " + herb.getLevelRequired() + " to grow "
							//+ herb.getSeedName().replaceAll(" seed", "") + ".");
					//return;
				//}
					...
					Server.getTaskScheduler()
							.schedule(new ScheduledTask(player, 3, Walkable.NON_WALKABLE, Stackable.NON_STACKABLE) {
								...
								public void execute() {
									...
									player.getSkills().addExperience(herb.getPlantingXp(), Skills.FARMING);
			...
			if (player.getFarmingState(id) >= State.SEEDED.getId()
				...
				if (!player.getItems().playerHasItem(FarmingConstants.WATERING_CAN, 1))
					...
					player.getActionSender().sendMessage("You water the herb, wait " + (player.getFarmingTime(id) * .6)
			...
			if (player.getFarmingState(id) == State.GROWTH.getId()) {
				if (player.getFarmingTime(id) > 0) {
					player.getActionSender().sendMessage("You need to wait another " + ((int) (player.getFarmingTime(id) * .6))
			...
			if (player.getFarmingState(id) == State.HARVEST.getId()) {
				if (player.getItems().freeSlots() < 1) {
					player.getActionSender().sendMessage("You need atleast 1 free space to harvest some herbs.");
					player.nextChat = -1;
					return;
				}
					...
					Server.getTaskScheduler().schedule(new ScheduledTask(player, 3, Walkable.NON_WALKABLE, Stackable.NON_STACKABLE) {
						...
						public void execute() {
							...
							if (player.getFarmingHarvest(id) <= 0) {
								player.getActionSender().sendMessage("The herb patch has completely depleted...");
								player.getPA().resetAnimation();
								resetValues(id);
								updateObjects();
								stop();
							...
							player.getSkills().addExperience(herb.getHarvestingXp(), Skills.FARMING);
						}
		...
		for(int i = 0; i < Farming.MAX_PATCHES; i++) {
	        if (player.getFarmingTime(i) > 0 && player.getFarmingState(i) == Farming.State.GROWTH.getId()) {
	        	...
	        	if(player.getFarmingTime(i) == 0) {
	        		...
	        		if(herb != null)
	        			player.getActionSender().sendMessage("Your farming patch of "+herb.getSeedName().replaceAll(" seed", "")+" is ready to be harvested.");
